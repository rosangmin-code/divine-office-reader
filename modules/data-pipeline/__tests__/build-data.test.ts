...
const contentAll = JSON.parse(fs.readFileSync(path.join(DATA_DIR, "content.json"), "utf-8")) as Record<string, { content: string; segments?: { type: string; text: string }[]; title: string; titleMn: string; level: number }>
...
// --- T-SEG: Segment quality tests ---

const sectionsWithContent = allIds.filter(id => contentAll[id].content.length > 0)

describe("T-SEG: 세그먼트 구조", () => {
  it("T-SEG-01: 실질적 콘텐츠가 있는 모든 섹션에 segments 배열 존재", () => {
    // Sections with only a day header (e.g., "Бямба гарагийн орой") have segments stripped
    const dayHeaderRe = /^(Ням|Даваа|Мягмар|Лхагва|Пүрэв|Баасан|Бямба)\s+гарагийн\s+(өглөө|орой|оройн)\s*$/
    const substantive = sectionsWithContent.filter(id => !dayHeaderRe.test(contentAll[id].content.trim()))
    const missing = substantive.filter(id => !contentAll[id].segments || contentAll[id].segments!.length === 0)
    expect(missing).toEqual([])
  })

  it("T-SEG-02: 중복 요일 헤더 세그먼트 없음", () => {
    const dayHeaderRe = /^(Ням|Даваа|Мягмар|Лхагва|Пүрэв|Баасан|Бямба)\s+гарагийн\s+(өглөө|орой|оройн)/
    const withDayHeader = sectionsWithContent.filter(id =>
      contentAll[id].segments?.some(s => dayHeaderRe.test(s.text))
    )
    expect(withDayHeader).toEqual([])
  })

  it("T-SEG-03: response 세그먼트가 존재하는 섹션 있음", () => {
    const withResponse = sectionsWithContent.filter(id =>
      contentAll[id].segments?.some(s => s.type === "response")
    )
    expect(withResponse.length).toBeGreaterThan(10)
  })

  it("T-SEG-04: doxology 세그먼트가 존재하는 섹션 있음", () => {
    const withDoxology = sectionsWithContent.filter(id =>
      contentAll[id].segments?.some(s => s.type === "doxology")
    )
    expect(withDoxology.length).toBeGreaterThan(10)
  })
})
