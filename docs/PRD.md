...
| F-XNAV | 교차 그룹 네비게이션 미지원 | **높** | 본문 내 페이지 참조(х. 874)가 plain text — 클릭 불가. 기도 중 시편집↔고유문↔찬미가 간 이동이 수동 사이드바에 의존 |
| F-FLOW | 기도 흐름 가이드 미지원 | 중 | 한 기도 세션(예: 아침기도)에서 필요한 섹션들을 순서대로 안내하는 기능 없음. 사용자가 직접 어떤 섹션을 읽어야 하는지 알아야 함 |
