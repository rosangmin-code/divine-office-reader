...
| 기능 | 상태 | 설명 |
|------|------|------|
| 계층 목차 네비게이션 | ✅ | 트리뷰 + 자동 펼치기 + 활성 항목 하이라이트 |
| 텍스트 검색 | ✅ | 제목 기반 검색 (2자 이상) |
| 폰트 크기 조절 | ✅ | 14-32px, ±2px 단위 |
| 다크 모드 | ✅ | 토글 + localStorage 영속 + FOUC 방지 |
| 브레드크럼 | ✅ | 현재 위치 경로 표시 |
| 설정 영속성 | ✅ | 마지막 읽기 위치, 폰트, 다크모드 기억 |
| Lazy loading | ✅ | 6개 그룹 분할, 스크롤 기반 pre-fetch |
| SSR | ✅ | bookmarks SSR + content lazy |
| 모바일 반응형 | ✅ | 사이드바 오버레이, 44px 터치 타겟 |
| 접근성 | ✅ | ARIA tree, 키보드 네비게이션, sr-only |
| 스켈레톤 로딩 | ✅ | 콘텐츠 로딩 중 pulse 애니메이션 |
| iOS viewport | ✅ | 100dvh 지원 |
...
| 이슈 | 심각도 | 상세 |
|------|--------|------|
| 전례 텍스트 구조 미표현 | 중 | 시편/찬가/독서/응답구 시각적 구분 없음 |
| 루브릭(지시문) 미구분 | 중 | 빨간색 사제 지시문 vs 검정색 기도문 구분 없음 |
| 키보드 단축키 없음 | 저 | Ctrl+F, 방향키 네비게이션 등 |
| 읽기 진행률 미표시 | 저 | 현재 섹션 위치/전체 비율 |
| 오프라인 미지원 | 중 | Service Worker/PWA 없음 |
